...
var GalactrumService = index.services.Galactrum;
...
describe('Node Functionality', function() {
  ...
  before(function(done) {
    ...
    testKey = orecore.PrivateKey(testWIF);
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      regtest = orecore.Networks.get('regtest');
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.galactrumd.height === 150) {
            node.services.galactrumd.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.galactrumd.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('galactrumd/hashblock', function(data) {
        bus.unsubscribe('galactrumd/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.galactrumd.height;
      node.services.galactrumd.on('tip', function(height) {
    ...
    describe('History', function() {
      ...
      before(function(done) {
        ...
        testKey2 = orecore.PrivateKey.fromWIF('cNfF4jXiLHQnFRsxaJyr2YSGcmtNYvxQYSakNhuDGxpkSzAwn95x');
        address2 = testKey2.toAddress(regtest).toString();

        testKey3 = orecore.PrivateKey.fromWIF('cVTYQbaFNetiZcvxzXcVMin89uMLC43pEBMy2etgZHbPPxH5obYt');
        address3 = testKey3.toAddress(regtest).toString();

        testKey4 = orecore.PrivateKey.fromWIF('cPNQmfE31H2oCUFqaHpfSqjDibkt7XoT2vydLJLDHNTvcddCesGw');
        address4 = testKey4.toAddress(regtest).toString();

        testKey5 = orecore.PrivateKey.fromWIF('cVrzm9gCmnzwEVMGeCxY6xLVPdG3XWW97kwkFH3H3v722nb99QBF');
        address5 = testKey5.toAddress(regtest).toString();

        testKey6 = orecore.PrivateKey.fromWIF('cPfMesNR2gsQEK69a6xe7qE44CZEZavgMUak5hQ74XDgsRmmGBYF');
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        var memAddress = orecore.PrivateKey().toAddress(node.network).toString();
        var tx = new Transaction();
        tx.from(unspentOutput);
        ...
        node.services.galactrumd.sendTransaction(tx.serialize(), function(err, hash) {
