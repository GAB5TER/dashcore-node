...
/**
 * Provides a friendly event driven API to galactrumd in Node.js. Manages starting and
 * stopping galactrumd as a child process for application support, as well as connecting
 * to multiple galactrumd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Galactrum(options) {
  if (!(this instanceof Galactrum)) {
    return new Galactrum(options);
  }
...
util.inherits(Galactrum, Service);

Galactrum.dependencies = [];


Galactrum.DEFAULT_MAX_TXIDS = 1000;
Galactrum.DEFAULT_MAX_HISTORY = 50;
Galactrum.DEFAULT_MAX_UTXO = 1000;
Galactrum.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Galactrum.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Galactrum.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Galactrum.DEFAULT_SPAWN_RESTART_TIME = 5000;
Galactrum.DEFAULT_SPAWN_STOP_TIME = 10000;
Galactrum.DEFAULT_TRY_ALL_INTERVAL = 1000;
Galactrum.DEFAULT_REINDEX_INTERVAL = 10000;
Galactrum.DEFAULT_START_RETRY_INTERVAL = 5000;
Galactrum.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Galactrum.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Galactrum.DEFAULT_INSTANTSEND_FEE = 10000;
Galactrum.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Galactrum.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Galactrum.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Galactrum.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Galactrum.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Galactrum.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Galactrum.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Galactrum.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Galactrum.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Galactrum.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Galactrum.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Galactrum.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Galactrum.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Galactrum.prototype._initCaches = function() {
...
Galactrum.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Galactrum.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Galactrum.prototype.getPublishEvents = function() {
  return [
    {
    ...
    {
      name: 'galactrumd/transactionlock',
      scope: this,
...
Galactrum.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'galactrumd/' + name, 'total:', this.subscriptions[name].length);
};

Galactrum.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'galactrumd/' + name, 'total:', this.subscriptions[name].length);
};

Galactrum.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (orecore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'galactrumd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Galactrum.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'galactrumd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Galactrum.prototype.unsubscribeAddressAll = function(emitter) {
...
Galactrum.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Galactrum.DEFAULT_CONFIG_SETTINGS;
...
Galactrum.prototype._parseGalactrumConf = function(configPath) {
  var options = {};
...
Galactrum.prototype._expandRelativeDatadir = function() {
...
Galactrum.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in galactrumd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in galactrumd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in galactrumd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './galactrum.conf');

  log.info('Using Galactrum config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseGalactrumConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseGalactrumConf(networkConfigPath));
  }
...
Galactrum.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of orecore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of orecore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of orecore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in galactrum.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that galactrumd is undergoing a reindex. ' +
             ...
             'of orecore-node services will start.');
    node._reindex = true;
  }
};

Galactrum.prototype._resetCaches = function() {
 ...
 * @private
 */
Galactrum.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Galactrum.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Galactrum.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Galactrum Daemon Ready');
          callback();
        });
      });
...
Galactrum.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 6269
  };
  if (this.node.network === orecore.Networks.testnet) {
    networkOptions.rpcport = 16269;
  }
  return networkOptions;
};

Galactrum.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === orecore.Networks.testnet) {
    networkPath = 'testnet3/galactrum.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/galactrum.conf';
    }
  }
  return networkPath;
};

Galactrum.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === orecore.Networks.testnet) {
...
Galactrum.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('galactrumd/hashblock', message.toString('hex'));
    }
  }

};

Galactrum.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Galactrum.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Galactrum Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Galactrum.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Galactrum.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('galactrumd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Galactrum.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = orecore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('galactrumd/rawtransaction', message.toString('hex'));
    }

    var tx = orecore.Transaction();
    tx.fromString(message);
    var txid = orecore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Galactrum.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = orecore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('galactrumd/transactionlock', message.toString('hex'));
    }

  }
};

Galactrum.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Galactrum.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Galactrum.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Galactrum.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Galactrum.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Galactrum Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Galactrum.DEFAULT_REINDEX_INTERVAL);

};

Galactrum.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Galactrum.prototype._stopSpawnedGalactrum = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/galactrumd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned galactrum process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean galactrum process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Galactrum.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedGalactrum(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting galactrum process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Galactrum process unexpectedly exited with code:', code);
        log.warn('Restarting galactrum child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Galactrum process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn galactrumd.'));
      }
...
Galactrum.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to galactrumd.'));
    }
 ...
 */
Galactrum.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Galactrum configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Galactrum.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Galactrum.prototype.syncPercentage = function(callback) {
  var self = this;
...
Galactrum.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, orecore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Galactrum.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = orecore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Galactrum.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Galactrum.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Galactrum.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in orecore is most recent to less recent
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Galactrum.prototype._getConfirmationsDetail = function(transaction) {
...
Galactrum.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Galactrum.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Galactrum.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A galactrum transaction id
 * @param {Function} callback
 */
Galactrum.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Galactrum.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof orecore.Address) {
...
Galactrum.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Galactrum.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Orecore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Galactrum.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = orecore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
  self.client.getBlockCount(function(err,response) {
    ...
    var hashes = [];
    var left = 0;
    var latestBlock = response.result;
    var right = latestBlock;

    function _findFirstBlock(left, right) {
      var middle = Math.floor((left + (right - 1))/2);
      self.getBlockHeader(middle, function (err, blockHeader) {
        if (err) {
          return callback(self._wrapRPCError(err));
        }

        if ( right <= left ) {
          _getBlockHashes(left);
        }
        else if (blockHeader.time < low) {
          _findFirstBlock(middle + 1, right);
        }
        else {
          _findFirstBlock(left, middle -1);
        }
      });
    }

    function _getBlockHashes(block) {
      self.getBlockHeader(block, function (err, blockHeader) {
        if (err) {
          return callback(self._wrapRPCError(err));
        }

        if ( blockHeader.time > high ) {
          callback(null, hashes);
        }
        else if ( block == latestBlock ) {
          hashes.push(blockHeader.hash);
          callback(null, hashes);
        }
        else {
          hashes.push(blockHeader.hash);
          _getBlockHashes(block + 1);
        }
      });
    }

    _findFirstBlock(left, right);
  });
};
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Galactrum.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
  var self = this;
  var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Galactrum.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Orecore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Galactrum.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Galactrum.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new orecore.Transaction(result.hex);
      var hash = orecore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.galactrumd.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
Galactrum.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Galactrum.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Galactrum.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Galactrum.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Galactrum.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Galactrum.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Galactrum.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Galactrum.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Galactrum.prototype.govCount = function (callback) {
  var self = this;
...
Galactrum.prototype.getSpork = function(callback){
  var self = this;
  var SporksList = {};
...
Galactrum.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Galactrum.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Galactrum.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.getInfo = function(callback) {
  var self = this;
  var info = {};
  this.client.getBlockchainInfo(function(err, response) {
    if (err) {
    ...
    info.testnet = info.network === 'livenet' ? false: true;
    info.proxy = "";
    info.errors = "";
    self.client.getNetworkInfo(function(err, response) {
      if (err) {
        return callback(self._wrapRPCError(err));
      }
      var result = response.result;
...
Galactrum.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Galactrum.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('galactrumd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('galactrumd process did not exit'));
      }
