...
/**
 * Will remove a service from orecore-node.json
...
/**
 * Will remove the Node.js service and from the orecore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The orecore-node configuration directory
...
function remove(options, done) {
  ...
  var orecoreConfigPath = path.resolve(configPath, 'orecore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(orecoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a orecore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to orecore-node.json
        removeConfig(orecoreConfigPath, service, next);
      });
    }, done
  );
}
