...
var BASE_PACKAGE = {
  description: 'A full Galactrum node built with Orecore',
...
/**
 * Will create a directory and galactrum.conf file for Galactrum.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createGalactrumDirectory(datadir, done) {
...
/**
 * Will create a base Orecore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The galactrum database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/orecore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Orecore Node directory, configuration file,
 * galactrum configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the orecore node configuration directory
 * @param {String} options.datadir - The path to the galactrum datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the orecore-node directory and configuration
    ...
    function(next) {
      // Setup the Galactrum directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createGalactrumDirectory(absDataDir, next);
      } else {
        next();
      }
    },
