...
/**
 * Will return the path and default orecore-node configuration. It will search for the
 * configuration file in the "~/.orecore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.orecore');
  var defaultConfigFile = path.resolve(defaultPath, './orecore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['galactrumd', 'web'];
