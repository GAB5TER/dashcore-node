...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for galactrumd is necessary for this upgrade with the "reindex=1" galactrum.conf option. \n' +
                 'There are changes necessary in both galactrum.conf and orecore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/GAB5TER/orecore-node/blob/master/docs/upgrade.md \n');
        ...
        servicesConfig: {
          galactrumd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/galactrumd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed orecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './orecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in orecore-node services directory
    ...
    if (servicePackage.orecoreNode) {
      serviceModule = service.name + '/' + servicePackage.orecoreNode;
    }
 ...
 * @param {Node} node - The Orecore Node instance
 */
